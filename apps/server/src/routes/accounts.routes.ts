...
router.get('/', authenticate, (req: AuthRequest, res: Response) => {
  const rows = db.prepare(
    'SELECT id, name, type, current_balance, available_balance, plaid_account_id, teller_account_id, last_synced_at FROM accounts WHERE user_id = ? ORDER BY type, name'
