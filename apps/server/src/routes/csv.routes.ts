...
router.post('/import', authenticate, upload.single('file'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const result = importCsvTransactions(req.userId!, rows, accountId);
    ...
    const aiResult = await classifyUnknownMerchantsWithAI(req.userId!, merchantNames);

    // After import + AI classification, detect pay schedule, recurring bills, and debt payments
    const paySchedule = detectPaySchedule(req.userId!);
    const recurringExpenses = getRecurringSummary(req.userId!);
    const detectedDebtPayments = detectDebtPayments(req.userId!);

    // Invalidate advisor cache so next visit generates fresh insights
    invalidateAdvisorCache(req.userId!);

    res.json({ ...result, aiClassified: aiResult.classified, paySchedule, recurringExpenses, detectedDebtPayments });
  } catch (err: any) {
...
router.post('/add-debt', authenticate, (req: AuthRequest, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO accounts (id, user_id, name, type, current_balance, interest_rate, minimum_payment)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(id, req.userId, name, type, bal, apr, minPay);
...
router.post('/import-debts', authenticate, upload.single('file'), (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      insert.run(crypto.randomUUID(), req.userId, name, type, balance, apr, minPay);
      imported++;
    }
...
router.post('/import-budget', authenticate, upload.single('file'), (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      upsert.run(crypto.randomUUID(), req.userId, category, limit);
      imported++;
    }
...
router.post('/import-bills', authenticate, upload.single('file'), (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      insert.run(crypto.randomUUID(), req.userId, name, amount, freq);
      imported++;
    }
