...
router.post('/', authenticate, (req: AuthRequest, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO incoming_events (id, user_id, name, estimated_amount, expected_date, is_recurring, recurrence_interval, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(id, req.userId, name, estimated_amount, expected_date || null, is_recurring ? 1 : 0, recurrence_interval || null, notes || null);
...
router.put('/:id', authenticate, (req: AuthRequest, res: Response) => {
  ...
  const existing = db.prepare('SELECT * FROM incoming_events WHERE id = ? AND user_id = ?').get(req.params.id, req.userId) as any;
  if (!existing) {
  ...
  const updated = db.prepare('SELECT * FROM incoming_events WHERE id = ?').get(req.params.id);
  res.json(updated);
});

router.delete('/:id', authenticate, (req: AuthRequest, res: Response) => {
  const result = db.prepare('DELETE FROM incoming_events WHERE id = ? AND user_id = ?').run(req.params.id, req.userId);
  if (result.changes === 0) {
    res.status(404).json({ error: 'not_found', message: 'Event not found' });
    return;
  }
